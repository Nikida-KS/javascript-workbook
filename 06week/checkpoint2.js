...
const pi = [];
const nums = strNums.forEach((item)=>{
  pi.push(parseInt(item));
})
...
const evens = pi.filter(item=> item % 2 == 0);
const sumEvens = evens.reduce((acc, item) => acc + item, 0)
console.log(sumEvens);

// Find the index of the first "Austin" value (the value plus its index equals 512)
const atxIdx  = pi.findIndex((item, index) => {
  return (item + index === 512)
});

console.log(`index: ${atxIdx}, value: ${pi[atxIdx]}`);
...
const weathers = weather.filter((item, index, array) => {
        return array.map(mapItem => mapItem.weather_state_name).indexOf(item.weather_state_name) === index;
    });

const weatherStates = weathers.map(item => item.weather_state_name)
